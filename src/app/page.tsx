...
const getBaseId = (sku: string): string | null => {
  const m = String(sku || "").match(/^(\d+)/);
  return m ? m[1] : null;
};

// non-stock / marketing (exclude from calculations)
const isMarketingSku = (sku: string): boolean =>
  /^(Catalog|Banner|FTflag|Base#|Display|Shelf)/i.test(String(sku || ""));
...
function CalculatorTab() {
  ...
  const [excludedMarketing, setExcludedMarketing] = useState<number>(0);
  const [missingRef, setMissingRef] = useState<number>(0);
  const [matchedByBase, setMatchedByBase] = useState<number>(0);
  const [matchedByAvg, setMatchedByAvg] = useState<number>(0);
...
const SORT_ORDER: (keyof CatSummary | 'category')[] = [
  ...
  const results = useMemo(() => {
    ...
    // агрегаты по baseId (если точного SKU нет в справочнике)
type BaseAgg = { avg_pcs_cbm: number; category: string };
const baseAgg = new Map<string, BaseAgg>();
const tmp = new Map<string, { sum: number; cnt: number; catFreq: Map<string, number> }>();

for (const r of ref) {
  const base = getBaseId(r.sku);
  if (!base) continue;

  const pcs = Number(r.pcs_cbm ?? 0);
  if (pcs > 0) {
    const t = tmp.get(base) ?? { sum: 0, cnt: 0, catFreq: new Map<string, number>() };
    t.sum += pcs;
    t.cnt += 1;
    tmp.set(base, t);
  }
  const catN = normalizeCategory(r.category ?? "Uncategorized");
  if (catN && catN !== "Uncategorized") {
    const t = tmp.get(base) ?? { sum: 0, cnt: 0, catFreq: new Map<string, number>() };
    t.catFreq.set(catN, (t.catFreq.get(catN) ?? 0) + 1);
    tmp.set(base, t);
  }
}
for (const [base, t] of tmp.entries()) {
  let bestCat = "Uncategorized";
  let bestN = 0;
  for (const [c, n] of t.catFreq.entries()) {
    if (n > bestN) { bestN = n; bestCat = c; }
  }
  baseAgg.set(base, { avg_pcs_cbm: t.cnt > 0 ? t.sum / t.cnt : 0, category: bestCat });
}
...
const skuRows = invRows
  .filter(ir => {
    const ok = !isMarketingSku(ir.sku);
    if (!ok) excludedMkt += 1;
    return ok;
  })
  .map(ir => {
    const sku = ir.sku;
    const qty_pcs = ir.qty_pcs || 0;

    const exact = refMap.get(sku);
    const baseId = exact ? null : getBaseId(sku);
    const baseRow = !exact && baseId ? refMap.get(baseId) : undefined;
    const agg = !exact && !baseRow && baseId ? baseAgg.get(baseId) : undefined;

    let match: "exact" | "base" | "avg" | "none" = "none";
    if (exact) match = "exact";
    else if (baseRow) match = "base";
    else if (agg) match = "avg";

    if (match === "none") missRef += 1;
    if (match === "base") byBase += 1;
    if (match === "avg") byAvg += 1;

    const pcs_cbm =
      (exact?.pcs_cbm ?? baseRow?.pcs_cbm ?? agg?.avg_pcs_cbm ?? 0);

    const categoryRaw =
      (exact?.category ?? baseRow?.category ?? agg?.category ?? "Uncategorized");

    const category = normalizeCategory(categoryRaw);
    const volume_cbm = qty_pcs * (pcs_cbm || 0);

    return { sku, qty_pcs, pcs_cbm, volume_cbm, category, match };
  });

setExcludedMarketing(excludedMkt);
setMissingRef(missRef);
setMatchedByBase(byBase);
setMatchedByAvg(byAvg);
      ...
      <Card>
        ...
        <CardContent className="grid grid-cols-1 gap-3 text-sm">
          <div className="flex gap-6 flex-wrap">
            ...
            <div>Нет в справочнике (даже по baseId): <b>{fmt0(missingRef)}</b></div>
            <div>Сматчено по baseId: <b>{fmt0(matchedByBase)}</b></div>
            <div>Сматчено по среднему baseId: <b>{fmt0(matchedByAvg)}</b></div>
            <div>Исключено marketing/non-stock: <b>{fmt0(excludedMarketing)}</b></div>
